...
class Word2GM(object):
    ...
    #####
    def find_nearest_dist(self, idx, cl, n=20):
        dist = np.dot(self.mus_n, self.mus_n[idx * self.num_mixtures + cl])
        sorted_idxs = dist.argsort()[::-1][:n]
        dist = sorted(dist)[::-1][:n]
        return [(sorted_idxs[x], dist[x]) for x in range(n)]
    ...
    def id2word_(self, idx):
        return "{}:{}".format(self.id2word[int(idx/self.num_mixtures)], idx%self.num_mixtures)
    ...
    def get_dist(self, word):
        a = self.get_idx(word)
        return self.dot(a, 0, a, 1)

    def filter(self, word, n, thresh=0.75):
        ind = self.word2id[word]
        res = [(self.id2word_(x[0]), x[1], -self.detA[x[0]]-3) for x in self.find_nearest_dist(ind, n, 20)]
        #res += [(self.id2word_(x[0]), x[1]) for x in self.find_nearest_dist(ind, 1, 20)]
        res = [x for x in res if x[1]>=thresh] if res[6][1]>thresh else res[:6]
        return res
    ####

    def word_dot(self, w1, w2):
        idx1 = self.word2id[w1]
        idx2 = self.word2id[w2]
        res = []
        for i in range(2):
            for j in range(2):
                res += [self.dot(idx1, i, idx2, j)]
        return res
