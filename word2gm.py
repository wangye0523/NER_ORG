...
class Word2GM(object):
    def __init__(self, save_path, ckpt_file=None, verbose=True):
        ...
        # if a ckpt file is provided, # TODO
    ...
    def negkl(self, w1, cl1, w2, cl2):
        ...
        #m1 = self.mus_n_multi[w1, cl1]
        #m2 = self.mus_n_multi[w2, cl2]
        m1 = self.mus[w1, cl1]
        m2 = self.mus[w2, cl2]
        epsilon = 1e-4
        ...
        # TODO: how to find if it's spherical or diagonal?
        if sph:
    ...
    def disdot(self, w1, w2):
        ...
        def partial_energy(cl1, cl2):
            ...
            _res += -0.5*np.sum(
                #np.dot(diff, np.dot(ss_inv, diff))
                diff*ss_inv*diff
            )
            return _res
    ...
    def wordsim_context(self, w1, c1, w2, c2, metric='dot_context', criterion='max', verbose=False):
        ...
        # TODO: How do we handle word not found?
        # TODO: handle different metric instead of dot?
        w1 = self.get_idx(w1)
        w2 = self.get_idx(w2)
        ...
        if metric == 'dot_context':
            ...
            # This approach is not using any context information
